...
(defun toggle-1 (grid x y)
  (let ((next-state (ecase (aref grid x y)
                      (:off :on)
                      (:on :off))))
    (setf (aref grid x y) next-state)))
...
(defun count-lights (grid state)
  (loop :with count := 0
        :for index :from 0 :upto (1- (apply '* (array-dimensions grid)))
        :when (eql state (row-major-aref grid index))
          :do (incf count)
