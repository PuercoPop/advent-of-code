...
(loop
  :for row :in +input+
  :for (max min) := (multiple-value-list (max-min-row row))
  :sum (- max min))


(defun ratio-for (row)
  (loop
    :for start :from 0 :below (length row)
    :for view := (subseq row start)
    :do (loop
          :for x := (car view)
          :for y :in (cdr view)
          :when (zerop (mod x y))
            :do (return-from ratio-for (/ x y))
          :when (zerop (mod y x))
            :do (return-from ratio-for (/ y x)))))

(loop
  :for row :in +input+
  :sum (ratio-for row)) ; => 250 (8 bits, #xFA, #o372, #b11111010)
